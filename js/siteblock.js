...
csapuntz.siteblock = (function () {
  var sbself = {
    read_options: function (stg) {
      ...
      if (!("rules" in opts)) opts.rules = "";

      if (!("allowed" in opts)) opts.allowed = 0;

      if (!("period" in opts)) opts.period = 1440;
    ...
    write_options: function (opts, stg) {
      ...
      if ("siteblock_list" in stg) delete stg["siteblock_list"];
    },
    ...
    newUsageTracker: function () {
      ...
      var check_reset = function (now) {
        if (last_end != -1 && now - last_end >= time_period - time_allowed) {
          time_used = 0;
        }
      };
      ...
      var get_time_used = function () {
        ...
        var time_adj = 0;
        if (last_start != -1) time_adj = time - last_start;
      ...
      var self = {
        start: function () {
          ...
          return function () {
            var end = time_cb();
            if (end > last_start) time_used += end - last_start;
    ...
    newSiteBlock: function () {
      ...
      var get_tracked_tabs = function () {
        ...
        for (var v in tabState) {
          if (v.substring(0, 3) == "Tab") t.push(Number(v.substring(3)));
        }
      ...
      var self = {
        ...
        getBlockedTabs: function () {
          ...
          for (i = 0; i < tabs.length; ++i) {
            ...
            if (ti.blocked) ret.push({ id: tabs[i], url: ti.url });
          }
          return ret;
        },
      }; // self =
